...
# # Two factor Vasicek model implementation: Temperature | Emissions
...
em_temp = pd.read_excel(r'C:\Users\Cathy\Documents\DOC\QF-UniBo\TESI\3_Data\em_temp.xlsx') #change path
#Year fractions
original_tau = em_temp['Tau']
#Zero rates from all the scenarios
original = em_temp['Original']
zrSSP119 = em_temp['SSP119']
zrSSP126 = em_temp['SSP126']
zrSSP245 = em_temp['SSP245']
zrSSP370 = em_temp['SSP370']
zrSSP585 = em_temp['SSP585']
...
ith Pool(processes=1) as pool:
    ...
    SSP126 = [pool.apply_async(model_curve, args = (Nt, 5678, params, original_tau[i], zrSSP126[i])) for i in parts]
    SSP245 = [pool.apply_async(model_curve, args = (Nt, 9467, params, original_tau[i], zrSSP245[i])) for i in parts]
    SSP370= [pool.apply_async(model_curve, args = (Nt, 7456, params, original_tau[i], zrSSP370[i])) for i in parts]
    SSP585 = [pool.apply_async(model_curve, args = (Nt, 3147, params, original_tau[i], zrSSP585[i])) for i in parts]
...
writer = pd.ExcelWriter('em_temp_results.xlsx', engine='xlsxwriter')
...
SSP = pd.read_excel(r'C:\Users\Cathy\Documents\DOC\QF-UniBo\TESI\3_Data\em_temp_results.xlsx')
...
fig, ax = plt.subplots() 
ax.plot(original_tau, zrSSP119,'o-', color = '#107d00', label = 'Original')
...
fig, ax = plt.subplots() 
ax.plot(original_tau, zrSSP126,'o-', color = '#16D92C', label = 'Original')
...
fig, ax = plt.subplots() 
ax.plot(original_tau, zrSSP245, 'o-', color='#FECE05', label = 'Original')
...
fig, ax = plt.subplots() 
ax.plot(original_tau, zrSSP370, 'o-',color='#E33630', label = 'Original')
...
fig, ax = plt.subplots() 
ax.plot(original_tau, zrSSP585, 'o-', color='#8F1E36', label = 'Original')
...
plt.suptitle('Temperature Benchmark Curves')
